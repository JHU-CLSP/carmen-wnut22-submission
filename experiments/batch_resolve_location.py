...
def agg(args):
    ...
    for filepath in partials:
        with open(filepath, 'rb') as f:
            obj = pickle.load(f)
        tot, res, tot_y, res_y, lc, lcy = obj
        total += tot
        resolved += res
        total_by_year += tot_y
        resolved_by_year += res_y
        location_counter += lc
        location_counter_by_year = combine_defaultdict_counters(location_counter_by_year, lcy)
    ...
    with open(os.path.join(args.output_path, 'locations.pkl'), 'wb') as f:
        pickle.dump(location_counter, f)

    with open(os.path.join(args.output_path, 'locations_by_year.pkl'), 'wb') as f:
        pickle.dump(location_counter_by_year, f)
...
if __name__ == "__main__":
    ...
    resolved_by_year = Counter()
    location_counter = Counter()
    location_counter_by_year = defaultdict(Counter)
    ...
    for filename in args.input_file:
        ...
        for tweet in reader.read_tweets():
            ...
            if resolution:
                ...
                location_counter[location.id] += 1
                location_counter_by_year[year][location.id] += 1
